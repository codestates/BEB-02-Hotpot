...
export default function Exchange() {
    ...
    const getPosts = async () => {
        await axios.get(getPostsURL)
            .then((res) => { dispatch(loadExchange(res.data)); })
            .then(() => { isloadingComplete(true); });
    }
    ...
    return (
        <div id='exchange-body'>
            ...
            <nav id="exchange-nav-container">
                ...
                <span id="search-wrap">
                    <input type="text" id="nft-search-input" placeholder="찾는 NFT를 검색해보세요"></input>
                    <SearchIcon fontSize='large' id="search-button" onClick={() => alert('버튼 테스트')}></SearchIcon>
                </span>
            </nav>
            <section className='nft-cards-wrap'>
                {posts.posts.map((post, idx) => <ExchangeNFT post={post} key={idx} />)}
